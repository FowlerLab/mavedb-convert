...
def parse_wt_sequence(wtseq, program, non_coding=False):
    ...
    if not constants.dna_re.fullmatch(wtseq):
        raise exceptions.InvalidWildTypeSequence(
            "Sequence {} is not a valid DNA sequence.".format(
                wtseq
            ))
...
def parse_docopt(docopt_args):
    ...
    parsed_kwargs['src'] = parse_src(
        docopt_args.get("<src>", None)
    )
    parsed_kwargs['dst'] = parse_dst(
        docopt_args.get("--dst", None)
    )
    ...
    parsed_kwargs['skip_header_rows'] = parse_numeric(
        docopt_args.get('--skip-header', 0),
        name='skip_header',
        dtype=int,
    )
    parsed_kwargs['skip_footer_rows'] = parse_numeric(
        docopt_args.get('--skip-footer', 0),
        name='skip_footer',
        dtype=int,
    )
